...
verify_connectivity() {
    ...
    for ((attempt=1; attempt<=max_attempts; attempt++)); do
        ...
        http_code=$(curl -s -o /dev/null -w '%{http_code}' --max-time 2 "http://localhost:$local_port/readiness" 2>/dev/null) || http_code="000"
...
cmd_restart_llama_stack() {
    ...
    oc apply -f "$MANIFEST_DIR/llama-stack.yaml"
