...
@pytest.mark.skip(reason="LCORE-1024: Missing RBAC validation for model override")
@pytest.mark.asyncio
async def test_query_v2_endpoint_rejects_model_override_without_permission(
    test_config: AppConfig,
    mock_llama_stack_client: AsyncMockType,
    test_request: Request,
    test_auth: AuthTuple,
    ...
    """Test that model override is rejected.

    This integration test verifies:
    - RBAC validation enforces MODEL_OVERRIDE permission
    - HTTPException is raised when permission missing
    - Model override in request triggers real validation
    ...
    # Set authorized_actions WITHOUT MODEL_OVERRIDE permission
    test_request.state.authorized_actions = {Action.QUERY}
    ...
    assert isinstance(exc_info.value.detail, dict)
    assert "permission" in exc_info.value.detail["response"].lower()

    # Verify no conversations were created (validation failed early)
    conversations = patch_db_session.query(UserConversation).all()
    assert len(conversations) == 0
