...
def restart_pod(container_name: str) -> None:
    """Restart a pod in OpenShift/Prow environment."""
    actual_pod_name = get_pod_name(container_name)

    if "lightspeed" in actual_pod_name:
        # restart-lightspeed includes port-forward restart
        try:
            ...
            print(f"Failed to restart pod {actual_pod_name}: {e}")
            raise
    else:
        # For other pods, wait for it to come back up
        print(f"Restarting pod {actual_pod_name}...")
        wait_for_pod_health(container_name, max_attempts=24)
